...
def get_user_input():
    grid = [[0 for _ in range(3)] for _ in range(3)]  # Initialize a 3x3 grid of 0s (walkable)

    print("Enter the grid values (0 for walkable, 1 for obstacle):")
    
    for i in range(3):
        for j in range(3):
            while True:
                try:
                    value = int(input(f"Enter value for cell ({i},{j}) (0 or 1): "))
                    if value not in [0, 1]:
                        print("Invalid input! Please enter 0 for walkable or 1 for obstacle.")
                        continue
                    grid[i][j] = value
                    break
                except ValueError:
