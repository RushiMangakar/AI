...
def solve_nq_limited(board, col, n, solutions, required_solutions):
    if col >= n:
        solution = []
        for row in board:
    ...
    for i in range(n):
        if is_safe(board, i, col, n):
            board[i][col] = 1
            if solve_nq_limited(board, col + 1, n, solutions, required_solutions):
                return True
            board[i][col] = 0
...
def find_solutions(n, required_solutions):
    ...
    solutions = []
    solve_nq_limited(board, 0, n, solutions, required_solutions)
    return solutions
